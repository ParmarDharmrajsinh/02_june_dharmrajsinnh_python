...
def notes(request):
    try:
        user=request.session.get("user")
        username=Userinfo.objects.get(email=user)
        if request.method=='POST':
            form = mynotesForm(request.POST, request.FILES)  # use form class
            if form.is_valid():
...
def contact(request):
    if request.method=='POST':
        form = ContactForm(request.POST)  # use form class
        if form.is_valid():
            form.save()
            print("Message Sent!")
...
def profile(request):
    ...
    cid=Userinfo.objects.get(id=uid)
    
    if request.method=='POST':
        form = UserinfoForm(request.POST, instance=cid)  # use form class for edit
        if form.is_valid():
            form.save()
...
def signin(request):
    if request.method=='POST':
        ...
        user=Userinfo.objects.filter(email=em,password=pa)
        uid=Userinfo.objects.get(email=em)
        print(uid.id)
        if user:
...
def signup(request):
    ...
    if request.method=='POST':
        ...
        if user:
            ...
            form = UserinfoForm(request.POST)  # use form class (was model)
            if form.is_valid():
                form.save()
